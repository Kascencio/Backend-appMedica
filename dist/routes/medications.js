...
const router = async (app) => {
    ...
    app.get('/', async (req, res) => {
        ...
        if (q.from || q.to)
            where.startDate = {};
        if (q.from)
            where.startDate.gte = new Date(q.from);
        if (q.to)
            where.startDate.lte = new Date(q.to);
    ...
    app.patch('/:id', async (req, res) => {
        ...
        const medData = {};
        if ('name' in body)
            medData.name = body.name;
        if ('dosage' in body)
            medData.dosage = body.dosage ?? null;
        if ('type' in body)
            medData.type = body.type ?? null;
        if ('frequency' in body)
            medData.frequency = body.frequency;
        if ('startDate' in body)
            medData.startDate = body.startDate ? new Date(body.startDate) : undefined;
        if ('endDate' in body)
            medData.endDate = body.endDate ? new Date(body.endDate) : null;
        if ('notes' in body)
            medData.notes = body.notes ?? null;
        const updated = await prisma.$transaction(async (tx) => {
            ...
            if (body.schedule) {
                ...
                const schedData = {};
                if ('frequency' in body.schedule)
                    schedData.frequency = body.schedule.frequency;
                if ('times' in body.schedule)
                    schedData.times = body.schedule.times;
                if ('daysOfWeek' in body.schedule)
                    Object.assign(schedData, body.schedule.daysOfWeek !== undefined ? { daysOfWeek: body.schedule.daysOfWeek } : {});
                if ('customRule' in body.schedule)
                    schedData.customRule = body.schedule.customRule ?? null;
                if ('timezone' in body.schedule)
                    schedData.timezone = body.schedule.timezone;
                if (existing) {
